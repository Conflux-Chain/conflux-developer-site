...
custom_edit_url: https://github.com/Conflux-Chain/conflux-developer-site/edit/master/docs/sending-tx/en/sending-tx-ultimate-guide.md
keywords:
  - transaction
---

Sending a transaction is the only way to interact with the Conflux network. Whether it is sending CFX or interacting with a contract (modifying the state on the contract chain), it can only be achieved through transactions.
...
However, if you want to know more bottom-level details, or if you encounter problems when sending transactions, this article may be helpful.
...
### Basic fields
`from`, `to`, `value` are the most basic fields of a transaction that corresponding to `originating account`, `destination account`, and `amount` of the transaction.

`from` is fairly easy to determine. You can simply select an external account (non-contract account) address with CFX balance. If the destination account of the transaction is a contract with sponsor, the initiating account does not even require a CFX balance. The balance of the account can be queried through RPC `cfx_getBalance` method.

`to` is the destination account of the transaction: if you just want to initiate a CFX transfer, then `to` can be directly set as the CFX destination account; if you need to change the contract status, `to` needs to be set as the contract address; if it is created for contract, `to` is left blank.

`value` represents the CFX transfer amount of a transaction, and it needs to set an integer with the unit of Drip.

### nonce
`nonce` is the number of transactions sent by an account and the execution sequence number of transactions sent by an account. It can be queried through the RPC method `cfx_getNextNonce`. It has the following characteristics:
...
4. Nonce cannot be skipped: Assume that the current nonce of an account is n. If the nonce of the transaction is set to a value that is >n, the transaction will not be executed until all transactions with nonce < n is executed, and then the transaction with nonce is n will be executed.
...
In the Conflux network, transaction fees are paid in CFX. It is specified by the transaction initiator through the `gas` and `gasPrice` of the transaction.

The `gas` field is used to specify the upper limit of the maximum amount of gas that can be paid when a transaction is executed. This can be understood as the `limit` of the gas that can be consumed by the transaction execution, that is, `gas(gasLimit)`. When a miner executes a transaction, many instructions are executed internally, and different instructions consume different amounts of gas. If the total amount of gas consumed by the transaction exceeds the `gas(gasLimit)` specified by the transaction,  the execution of the transaction will fail. Regular CFX transfer transaction consumes `21000` gas. If interacting with a contract, the gas consumption will depend on the complexity of the contract's corresponding opcode, which can be estimated using the `cfx_estimateGasAndCollateral` method. This method will return two gas-related fields:  gasUsed and `gasLimit`. The gasUsed is th e actual amount of gas consumed by the transaction execution at the time of estimation. The `gasLimit` is the `gas(gasLimit)` value set by transaction sending recommendation (slightly larger than `gasUsed`) to avoid transaction failure caused by inaccurate (smaller) estimation. Only 25% of the exceeding actual gas (compared to the `gas(gasLimit)` value) will be returned. Therefore it is important to set the transaction `gas(gasLimit)` appropriately.

`gasPrice` is the amount of CFX that the transaction initiator is willing to pay per gas. The unit is Drip. Thus, the calculation of the gas fee charged for transaction execution is `gas(gasLimit)` * `gasPrice`.
...
In addition to transaction fees, in the Conflux network, if new storage space is occupied after the transaction is executed, some CFX are pledged for that space occupation. The annualized %4 interest generated by the pledged CFX will be paid to the miner. If the occupied space is released, the pledged CFX will be returned. For every 1KB of new space taken, 1 CFX is pledged. The `storageLimit` field is used to specify the upper limit of the space (in bytes) occupied by a transaction execution. `storageLimit` specifies that any more than the actual value used will be returned without charge.

Therefore, when sending a transaction, you need to ensure the sending account has enough balance to pay: `value + storageLimit * (10^18/1024) + gas * gasPrice`. If the balance is insufficient, the transaction may get stuck in the transaction pool. If the transaction is interacting with the contract, and the contract has a sponsor, you only need to ensure that the balance is enough to pay for value.
...
* Contract call transaction: The `data` field is used to store the input data for the contract to call. The data is usually the contract method and data after parameter abi encoding. (should be left blank when calling fallback function of the contract)
...
## How to determine a transaction is confirmed
In the blockchain, a block can be reverted after being executed. Accordingly, a transaction can also be reversed after being executed. The status of the transaction will not be changed only after the block is confirmed.
...
According to the different stages of the potential transaction failure, the failure can be situated into the following three types:
...
### Stucked in the transaction pool
...
### Executed failed
Execution failure is usually due to an error occurred during the execution process of the contract, which then caused the transaction failure. Such errors are mostly caused by contract execution failures or errors returned when estimating gas cost through the estimate interface.
...
5. `VmError(BadInstruction` Contract deployment failed
6. `Vm reverted, ` The contract execution failed with no details provided.


## Difference between Conflux and Ethereum

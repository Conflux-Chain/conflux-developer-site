...
`value` 表示一笔交易的 CFX 转账金额，需设置一个单位为 Drip 的整数。
...
```
交易被打包之后不会立刻执行，这时交易的 `status` 为 `null`。延迟 4 个 Epoch 后，交易的 status 应该变为 `0x0` 或 `0x1`, `0x2`，`0x0` 表示成功，`0x1` 为失败, `0x2` 表示跳过。
...
## 跟以太坊的区别
Conflux 交易跟以太坊相比多了 `chainId`, `storageLimit`, `epochNumber` 三个字段。并且交易的 rlp 编码组装方式不同。除此之外 ecdsaSign 签名后的 v 值在 Conflux 不做特殊修改，以太坊中则会对 v 值有一些特殊处理。